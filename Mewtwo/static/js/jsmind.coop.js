...
(function ($w, temp) {
    ...
    Jm.coop.prototype = {
        ...
        // websocket 地址
        coop_ws_url: null,
        // 标记当前客户端（同一个IP可能打开多个浏览器，需要用client_uuid区分）
        client_uuid: null,
        // 当前脑图ID
        coop_mind_id: null,
        // 上一次操作日志ID
        ...
        _init_properties: function (_jm) {
            // 设置属性
            this.client_uuid = Jm.util.uuid.newid()
        ...
        _build_evt_param: function () {
            return {'client_uuid': this.client_uuid, 'log_uuid': Jm.util.uuid.newid(), 'coop_mind_id': this.coop_mind_id}
        },
        // 调用jsmind接口
        ...
        _handle_msg_inner(msg_json, is_batch) {
            ...
            // 校验 client_uuid
            const client_uuid = msg_json.client_uuid
            if (!client_uuid) {
                return
            }
            ...
            if (log_id > this.coop_mind_log_id) {
                ...
                if (is_batch || client_uuid !== this.client_uuid) {
            ...
            // 回放
            if (client_uuid === this.client_uuid) {
